...
for k = 1:3
    figure(k+1);
    sTraffic = Traffic * (1.1 + (k-1)*0.1);
    % Tính trọng số các node
    Node_Weight=ones(1,length(Node));
    for i = 1:length(Node)
        Node_Weight(i) = sum(sTraffic(i,:));
    end
    ...
    for i=1:length(Node)
        ...
        if ((~ismember(i,Backbone_Node)) && (~ismember(i,Member_Node))) 
            ...
            scatter(x_Node(i),y_Node(i),40,'y','filled');
        end
    end
        ...
        for j=1:i
            if (sTraffic(i,j) ~= 0)
                ...
                if (a == b)
                    ...
                    Traffic_Backbone(a, b) = Traffic_Backbone(a, b) + sTraffic(i, j);
                    Traffic_Backbone(b, a) = Traffic_Backbone(b, a) + sTraffic(i, j);
                end
            end
        end
    end
